...
void AEnemy::BeginPlay()
{
	Super::BeginPlay();
	Health = MaxHealth;
	AgrosSphere->OnComponentBeginOverlap.AddDynamic(
		this, 
	...
	if (EnemyController) {
		...
		EnemyController->
			...
			SetValueAsBool(FName("CanAttack"),
				true);
...
void AEnemy::StoreHitNumber(UUserWidget* HitNumber, FVector Loaction)
{
	HitNumbers.Add(HitNumber, Loaction);
...
void AEnemy::AgroSphereOverlap(UPrimitiveComponent* OverlappedComponent, AActor* OtherActor, UPrimitiveComponent* OtherComp, int32 OtherBodyIndex, bool bFromSweep, const FHitResult& SweepResult)
{
	...
	auto Character = Cast<AUnrealSuviverGameCharacter>(OtherActor);
	if (Character) {
		if (EnemyController) {
...
{
	if (OtherActor == nullptr) return;
	auto SurviverCharacter = Cast<AUnrealSuviverGameCharacter>(OtherActor);
	if (SurviverCharacter) {
		bInAttackRange = true;
		if (EnemyController) {
			EnemyController->GetBlackboardComponent()->SetValueAsBool(
				TEXT("InAttackRange"),
...
FName AEnemy::GetAttackSectionname()
{
	...
	UE_LOG(LogTemp, Warning, TEXT("GetAttackSectionname:: %s"),*SectionName.ToString());
	return SectionName;
}

void AEnemy::DoDamage(AUnrealSuviverGameCharacter* Victim) {
...
void AEnemy::SpawnBlood(AUnrealSuviverGameCharacter* Victim, FName SocketName)
{
...
void AEnemy::StunCharacter(AUnrealSuviverGameCharacter* Victim) {
...
void AEnemy::ReSetCanChase()
{
	bCanChase = true;
	if (EnemyController) {
		EnemyController->
			GetBlackboardComponent()->
			SetValueAsBool(FName("CanChase"),
				true);
	}
}

void AEnemy::SetCanNotChase()
{
	bCanChase = false;
	if (EnemyController) {
		EnemyController->
...
float AEnemy::TakeDamage(float DamageAmount, FDamageEvent const& DamageEvent, AController* EventInstigator, AActor* DamageCauser)
{
	if (EnemyController) {
		EnemyController->
			GetBlackboardComponent()->
			SetValueAsObject(FName("Target"), DamageCauser);
	}
	...
	const float Stunned = FMath::FRandRange(0.f, 1.0f);
